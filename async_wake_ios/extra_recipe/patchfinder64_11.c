...
#define INSN_STR  0xF9016128, 0xFFFFFFFF
...
#define INSN_CBNZ 0x35000D78, 0xFFFFFFFF
#define INSN_ADRP 0xD0002317, 0xFFFFFFFF
...
find_rootvnode(void)
{
    ...
    if (!ref) {
        printf("[ERROR]: could not find 'Volume mounted on a device with invalid major number'\n");
        return 0;
    }
    printf("ref str is at :%llx\n", (uint64_t) ref);
    
    ref -= kerndumpbase;
    
    cbnz = step64(kernel, ref, 91, INSN_CBNZ);
    if (!cbnz) {
        return 0;
    }
    printf("cbnz is at :%llx\n", (uint64_t) cbnz);
    
    adrp = step64(kernel, cbnz, 52, INSN_ADRP);
    if (!adrp) {
        return 0;
    }
    printf("adrp is at :%llx\n", (uint64_t) adrp);

    val = calc64(kernel, adrp, adrp + 16, 8);
    if (!val)
        return 0;
    printf("val(rootvnode) is at: %llx\n", val);
...
//0xFFFFFFF0075B6080-0xFFFFFFF00718DBD8
addr_t
...
find_trustcache(void)
{
    ...
    if (!ref) {
        ...
        addr_t str = step64(kernel, first_bl, 25, INSN_STR);
        if (!str)
            return 0;
